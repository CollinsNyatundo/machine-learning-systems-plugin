...
* **c5.xlarge (CPU):** Rate = $0.17/hr | Throughput = 50 req/s | Cost/1M = $0.94
* **g4dn.xlarge (T4 GPU):** Rate = $0.53/hr | Throughput = 400 req/s | Cost/1M = $0.37
* **p3.2xlarge (V100 GPU):** Rate = $3.06/hr | Throughput = 1,200 req/s | Cost/1M = $0.71
...
\(\frac{\sum T_{\text{compute}}}{\text{Wall Clock Time} \times \text{Resource Count}}\)
...
\( W_{p99} \approx \frac{\text{service time}}{1-\rho} \cdot \ln\left(\frac{1}{1-0.99}\right) \approx \frac{4.6 \cdot \text{service time}}{1-\rho} \)
...
\( \mathbb{E}[L_{\text{lat,wait}}] = \frac{T}{2} \)
...
\[ \frac{\frac{O}{R_{\text{peak}} \cdot \eta_{\text{hw}}} + L_{\text{lat}}}{T} = \underbrace{5 \text{ ms} + 0.6B \times \text{ms}}_{S(B)} \]
...
\[
\text{Lag} = \underbrace{\text{Overhead}}_{\text{Constant time}} + \underbrace{\text{Compute}}_{\text{Batch size-dependent time}}
\]
