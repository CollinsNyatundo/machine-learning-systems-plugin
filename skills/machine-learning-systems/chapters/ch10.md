...
\[ \min_{\hat{W}} \; \mathcal{L}(\hat{W}) \ \text{subject to} \ \|\hat{W}\|_{0} \le k \]
...
\(x_q = \operatorname{clamp}\big(\operatorname{round}(x/s + z), 0, 2^{b}-1\big)\)
...
\[ S = \frac{ \left\| \mathbf{1}_{\{T_{ij} = 0\}} \right\|_0 }{ m \times n } \]
